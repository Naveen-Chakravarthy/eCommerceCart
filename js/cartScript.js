var shoppingCart = (function() {
  ...
  obj.getTotalPriceAfterDiscount = function(item, count, price) {
    ...
    if(item === "A" && count >= 3)
    {
      return Number((price * count) - 15).toFixed(2);
    }
    else if (item === "B" && count >= 2)
    {
      return Number((price * count) - 5).toFixed(2);
    }
    else
    {
...
function displayCart() {
  ...
  for(var i in cartArray) {
    output += "<tr>"
      ...
      + "<input type='number' style='width: 200px;' class='item-count form-control' data-name='" + cartArray[i].name + "' value='" + cartArray[i].count + "'>"
